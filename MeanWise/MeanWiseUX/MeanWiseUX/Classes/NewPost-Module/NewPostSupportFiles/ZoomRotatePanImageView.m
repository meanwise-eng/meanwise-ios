...
//    }
//    
//    CGRect frame = CGRectInset(self.bounds, widthExpo, heightExpo);
//    
...
- (IBAction) handlePinch:(UIPinchGestureRecognizer*)recogniser {
    [delegate performSelector:onMovementCallBackFunc withObject:nil afterDelay:0.001];
...
- (IBAction) handlePan:(UIPanGestureRecognizer*)recogniser {
    ...
    if (recogniser.state == UIGestureRecognizerStateBegan || recogniser.state == UIGestureRecognizerStateChanged) {
        ...
        [self handlePanningWithPoint:translatedCenter];
    }
...
-(void)handlePanningWithPoint:(CGPoint)point
{
