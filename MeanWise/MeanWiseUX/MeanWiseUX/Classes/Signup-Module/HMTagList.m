...
-(void)updateUI
{
    ...
    for(int i=0;i<[arrayTags count];i++)
    {
        ...
        HMTagLabel *tag=[[HMTagLabel alloc] initWithFrame:CGRectMake(x, y, 200, 30)];
        [tag setUpString:[[arrayTags objectAtIndex:i] valueForKey:@"text"] withHeight:30];
        [self addSubview:tag];
        [tag setDelegate:self andFunc:@selector(tagRemoved:)];


        CGRect getFrame=[tag getFrame];
        tag.frame=CGRectMake(x, y, getFrame.size.width+40, getFrame.size.height);

        x=x+getFrame.size.width+40+5;
...
-(void)setUpString:(NSString *)string withHeight:(int)height
{
    ...
    label.font=[UIFont fontWithName:k_fontRegular size:18];

    label.text=[NSString stringWithFormat:@"   %@",string];
    label.numberOfLines = 0; // allows label to have as many lines as needed
    [label sizeToFit];
    
    label.frame=CGRectMake(0, 3, label.frame.size.width, label.frame.size.height);
    ...
    UILabel *lbl=[[UILabel alloc] initWithFrame:CGRectMake(label.frame.size.width+5, 0, height, height)];
    lbl.backgroundColor=[UIColor colorWithRed:1 green:1 blue:1 alpha:0.0f];
    [self addSubview:lbl];
    lbl.text=@"X";
    lbl.textAlignment=NSTextAlignmentCenter;
    lbl.textColor=[UIColor whiteColor];
    lbl.font=[UIFont fontWithName:k_fontRegular size:15];
