...
-(void)cleanUp
{
    self.mediaView.URL = nil;
    self.statusLBL.text = nil;
    self.profLBL.text = nil;
    self.tagName.text = nil;
    self.timeLBL.text = nil;
    self.likeCountLBL.text = nil;
    self.commentCountLBL.text = nil;
}
...
-(void)setURL:(NSString *)stringURL
{
    ...
    [self.mediaView setAssetURL:[NSURL URLWithString:stringURL]];
...
-(void)dismissURL
{
    ...
    [self.mediaView setAssetURL:nil];
...
-(id)initWithFrame:(CGRect)frame
{
    ...
    if(self)
    {
        ...
        isVideo = NO;
//        
        self.postIMGVIEW = [[ASDisplayNode alloc] init];
        [self.postIMGVIEW setFrame:self.bounds];
       ...
       // self.postIMGVIEW.image=[UIImage imageNamed:@"post_3.jpeg"];
        ...
        [self.contentView addSubview:self.postIMGVIEW.view];
        ...
        self.mediaView = [[ASVideoNode alloc] init];
        [self.mediaView setFrame:self.postIMGVIEW.frame];
        self.mediaView.delegate = self;
        self.mediaView.shouldAutoplay = YES;
        self.mediaView.shouldAutorepeat = YES;
        self.mediaView.gravity = AVLayerVideoGravityResizeAspectFill;
        self.mediaView.shouldRenderProgressImages = YES;
        self.mediaView.shouldCacheImage = YES;
        [self.postIMGVIEW addSubnode:self.mediaView];
...
-(void)profileBtnClicked:(NSDictionary *)userDict
{
    ...
    if(![screenIdentifier isEqualToString:@"PROFILE"])
    {
        ...
        NewProfileView *compo=[[NewProfileView alloc] initWithFrame:rect];
        [compo setUpProfileObj:userDict];
        [compo setDelegate:self andFunc1:@selector(ProfilecloseBtnClicked:)];
        [compo setUpFromView:@"profileButton"];
        
        rect.size.width = 60;
        rect.size.height = 60;
        
        [UIView animateWithDuration:0.3 animations:^{
            self.profileIMGVIEW.frame = rect;
        }];
...
-(void)ProfilecloseBtnClicked:(id)sender
{
    ...
    [UIView animateWithDuration:0.5 delay:0 usingSpringWithDamping:0.5 initialSpringVelocity:0.0 options:0 animations: ^{
...
-(void)setDataObj:(APIObjects_FeedObj *)dict;
{
    ...
    if(dict.mediaType.intValue!=0)
    {
        ...
        self.mediaView.URL = [NSURL URLWithString:dict.image_url];
        
        NSLog(@"ADDING IMAGE TO CELL %@ ", dict.image_url);
    }
    
    int colorNum;
    
    if(dict.colorNumber.intValue){
        colorNum = dict.colorNumber.intValue;
    }
    else{
        colorNum = arc4random_uniform(12);
    }
    ...
    if(dict.mediaType.intValue==2)
    {
        ...
        NSLog(@"ADDING VIDEO TO CELL %@ ", dict.video_url);
...
- (void)setImageOffset:(CGPoint)imageOffset
{
    ...
    self.mediaView.frame = offsetFrame;
}
...
-(void)setFrameX:(CGRect)frame;
{
   // self.postIMGVIEW.frame=frame;
    ...
    self.deleteBtn.frame=CGRectMake(frame.size.width-50, frame.size.height-195-heightBottom-70, 50, 50);
    
    self.timeLBL.frame=CGRectMake(frame.size.width-65, self.frame.size.height-heightBottom-30, 50, 30);
    self.shareBtn.frame=CGRectMake(frame.size.width-50, frame.size.height-heightBottom-90, 50, 50);
    self.commentBtn.frame=CGRectMake(frame.size.width-50, frame.size.height-140-heightBottom, 50, 50);
    self.commentCountLBL.frame=CGRectMake(frame.size.width-50, frame.size.height-155-heightBottom, 50, 30);
    self.likeBtn.frame=CGRectMake(frame.size.width-50, frame.size.height-195-heightBottom, 50, 50);
    self.likeCountLBL.frame=CGRectMake(frame.size.width-50, frame.size.height-210-heightBottom, 50, 30);
 ...
 //   [self.contentView addSubview:commmentComp];
   ...
   // [commmentComp setUp];
    ...
    if(mediaType==0)
    {
        ...
        self.mediaView.backgroundColor=[Constant colorGlobal:Cnumber];
        self.postIMGVIEW.backgroundColor=[Constant colorGlobal:Cnumber];
