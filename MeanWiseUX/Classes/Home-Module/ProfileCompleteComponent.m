...
@implementation ProfileCompleteComponent
...
-(void)setProfileDataCheck
{
    ...
    if (![obj.profile_story_title isEqualToString:@""]) {
        blnIntro = YES;
    }
    else{
        blnIntro = NO;
    ...
    if (![obj.profession isEqualToString:@""]) {
        blnProf = YES;
    }
    else{
        blnProf = NO;
    ...
    CompleteProfileItem* cover = [[CompleteProfileItem alloc] initWithName:@"Cover Photo" setImage:[UIImage imageNamed:@"coverPhotoBtn"] setStatus:blnCover];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell == nil) {
        ...
        if(item.status){
            cell.itemStatus.image = [UIImage imageNamed:@"doneBtn"];
            [cell.itemStatus setFrame:CGRectMake(self.bounds.size.width - 50, 10, 23, 13)];
            
        }
        else{
            cell.itemStatus.image = [UIImage imageNamed:@"notDoneBtn"];
