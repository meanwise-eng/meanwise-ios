...
-(void)responseReceived:(APIResponseObj *)obj
{
    ...
    if([obj.response isKindOfClass:[NSArray class]])
    {
        ...
        arrayTopics=[obj.response mutableCopy];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        string = @"*";
        cell.nameLBL.font = [UIFont fontWithName:k_fontBold size:14];
    }
    else{
...
{
    NSDictionary *dictAttributes=@{NSFontAttributeName:[UIFont fontWithName:k_fontRegular size:12]};
    ...
    return CGSizeMake(calCulateSizze.width+20, self.frame.size.height-5);
}
