...
-(void)setUpWithCellRect:(CGRect)rect
{
    ...
    postIMGVIEW=[[UIImageView alloc] initWithFrame:containerView.bounds];
    postIMGVIEW.image=[UIImage imageNamed:@"CoverPhoto.jpg"];
    ...
    coverView=[[Profile_CoverView alloc] initWithFrame:CGXRectMake(0, CGX_ScreenMaxHeight()*0, CGX_ScreenMaxWidth(), CGX_ScreenMaxHeight())];
    ...
    [coverView setTarget:self andCloseProfile:@selector(dismissThisController)];
    
    
    bioView=[[Profile_BioView alloc] initWithFrame:CGXRectMake(0, CGX_ScreenMaxHeight(), CGX_ScreenMaxWidth(), CGX_ScreenMaxHeight())];
    [scrollView addSubview:bioView];
    [bioView setUpUIComponents];
    [bioView setTarget:self andCloseProfile:@selector(dismissThisController)];
    
    
    videoView=[[Profile_VideoView alloc] initWithFrame:CGXRectMake(0, CGX_ScreenMaxHeight()*2, CGX_ScreenMaxWidth(), CGX_ScreenMaxHeight())];
    [scrollView addSubview:videoView];
    [videoView setUpUIComponents];
    [videoView setTarget:self andCloseProfile:@selector(dismissThisController)];

    portfolioView=[[Profile_PortfolioView alloc] initWithFrame:CGXRectMake(0, CGX_ScreenMaxHeight()*3, CGX_ScreenMaxWidth(), CGX_ScreenMaxHeight())];
    [scrollView addSubview:portfolioView];
    [portfolioView setUpUIComponents];
    [portfolioView setTarget:self andCloseProfile:@selector(dismissThisController)];
    ...
    introBlockView=[[UIView alloc] initWithFrame:self.bounds];
    [containerView addSubview:introBlockView];
    
    
    UILabel *introProgress=[[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 50)];
    [introBlockView addSubview:introProgress];
    introProgress.text=@"Tap anywhere to turn off the auto-play.";
    introProgress.backgroundColor=[UIColor colorWithWhite:0.0 alpha:0.5f];
    introProgress.textColor=[UIColor whiteColor];
    introProgress.numberOfLines=1;
    introProgress.textAlignment=NSTextAlignmentCenter;
    introProgress.font=[UIFont fontWithName:k_fontRegular size:15];
    introProgress.center=CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2-100);
    introProgress.clipsToBounds=YES;

    [UIView animateKeyframesWithDuration:1.0 delay:2.0 options:UIViewKeyframeAnimationOptionCalculationModeLinear animations:^{
        
        introProgress.frame=CGRectMake(0, 20, self.frame.size.width, 20);
    ...
    [self performSelector:@selector(setUpForIntro) withObject:nil afterDelay:0.05f];
}
...
-(void)zoomDownOut
{
    [delegate performSelector:closeSelector withObject:nil afterDelay:0.01];
}
-(void)setImage:(NSString *)string andNumber:(NSIndexPath *)indexPath
{
    postIMGVIEW.image=[UIImage imageNamed:string];
    [coverView setCoverPhoto:string andTitle:@""];
...
{
    [self closeThisView];
   //D [self dismissViewControllerAnimated:YES completion:^{
    //}];
...
@implementation Profile_ModelView
...
@implementation Profile_CoverView
...
{
   //D ProfileViewController *vc=(ProfileViewController *)[self viewController];
   //D [vc scrollToBioView];
    
}
-(void)closeBtnClicked:(id)sender
{
    
    [delegate performSelector:closeProfileFunc withObject:nil afterDelay:0.01];

  //D  ProfileViewController *vc=(ProfileViewController *)[self viewController];
  //D  [vc dismissViewControllerAnimated:YES completion:nil];
...
@implementation Profile_BioView
...
-(void)setUpUIComponents
{
    ...
    {
        Profile_BioItemView *view=[[Profile_BioItemView alloc] initWithFrame:CGXRectMake(CGX_ScreenMaxWidth()*i, 0, CGX_ScreenMaxWidth(), CGX_ScreenMaxHeight())];
...
@implementation Profile_VideoView
...
@implementation Profile_PortfolioView
...
-(void)setUpUIComponents
{
    ...
    for(int i=0;i<3;i++)
    {
        Profile_PortfolioItemView *view=[[Profile_PortfolioItemView alloc] initWithFrame:CGXRectMake(CGX_ScreenMaxWidth()*i, 0, CGX_ScreenMaxWidth(), CGX_ScreenMaxHeight())];
        [scrollView addSubview:view];
...
-(void)updateScollerWithDelta:(CGFloat)delta;
{
    for(int i=0;i<[pagesArray count];i++)
    {
        ...
        Profile_PortfolioItemView *item=(Profile_PortfolioItemView *)[pagesArray objectAtIndex:i];
...
@implementation Profile_PortfolioItemView
...
@implementation Profile_BioItemView
