...
-(void)setUp
{
    ...
    NSArray *imgArray=[NSArray arrayWithObjects:@"be-yourself",@"you-belong",@"real-connections", nil];
    NSArray *titleArray=[NSArray arrayWithObjects:@"GET TO BE YOURSELF",@"FIT WHERE YOU BELONG",@"MAKE REAL CONNECTIONS", nil];
    NSArray *detailArray=[NSArray arrayWithObjects:
                          ...
                          @"Say goodbye to your old-fashioned resume. Apply for the job you want in seconds.", nil];
    ...
    for(int i=0;i<3;i++)
    {
        ...
        UIImageView *imageView=[[UIImageView alloc] initWithFrame:CGRectMake((self.frame.size.width-self.frame.size.width*0.8)/2, 100,self.frame.size.width*0.8, view.frame.size.width*0.8)];
         imageView.image=[UIImage imageNamed:[NSString stringWithFormat:@"%@", [imgArray objectAtIndex:i]]];
        ...
        [titleLBL setFont:[UIFont fontWithName:k_fontBold size:17]];
        
        
        UILabel *titleLBL2=[[UILabel alloc] initWithFrame:CGRectMake(40, self.frame.size.height*0.67+25, self.frame.size.width-80,10)];
    ...
    component=[[LoginComponent alloc] initWithFrame:CGRectMake(3*self.frame.size.width, 0, self.frame.size.width, self.frame.size.height)];
    [component setUp];
    ...
    skipBtn.titleLabel.font=[UIFont fontWithName:k_fontSemiBold size:13];
