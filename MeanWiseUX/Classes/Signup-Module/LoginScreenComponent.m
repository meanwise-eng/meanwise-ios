...
-(void)setUp
{
    ...
    forgetPassBtn=[[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width-180, 20,180, 50)];
    ...
    forgetPassBtn.titleLabel.font=[UIFont fontWithName:k_fontSemiBold size:13];
    ...
    [passField addTarget:self
                   action:@selector(passTextChange:)
         forControlEvents:UIControlEventEditingChanged];
...
-(void)emailTextChange:(id)sender
{
    ...
    if(![passField.text isEqualToString:@""] && ![emailField.text isEqualToString:@""] && !emailValidationSign.isHidden)
    {
